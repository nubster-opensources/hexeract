//! Integration tests for the PostgreSQL backend of `hexeract-scheduler-sql`.
//!
...
//!
//! ```sh
//! cargo test -p hexeract-scheduler-sql --features postgres --test integration_pg -- --ignored
//! ```
...
use hexeract_scheduler_sql::PgScheduleStore;
...
async fn setup() -> (ContainerAsync<Postgres>, PgScheduleStore) {
    ...
    let ddl = schema_ddl(Dialect::Postgres, TABLE).expect("schema ddl");
    sqlx::raw_sql(&ddl)
        .execute(&pool)
        .await
        .expect("schema apply");
    let store = PgScheduleStore::new(pool, TABLE).expect("store");
    (container, store)
}
...
/// `FOR UPDATE SKIP LOCKED`: every occurrence is claimed exactly once.
#[tokio::test]
#[ignore = "runs in the integration workflow"]

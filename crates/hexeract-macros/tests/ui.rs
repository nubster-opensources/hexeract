...
fn ui() {
    ...
    // #451: the receiver form is validated, so every rejected shape must be pinned
    t.compile_fail("tests/ui/fail_receiver_owned.rs");
    t.compile_fail("tests/ui/fail_receiver_mut_owned.rs");
    t.compile_fail("tests/ui/fail_receiver_mut_reference.rs");
    t.compile_fail("tests/ui/fail_receiver_typed.rs");

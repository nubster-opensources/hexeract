...
- `hexeract-bus`: `RequestClient`, a synchronous-over-async RPC client over `Transport`. `request` and `request_with_timeout` publish a `Request` on a fresh causal chain; `request_in` and `request_in_with_timeout` publish it on the causal chain of a caller-supplied `HandlerContext` instead, so a downstream call inherits its correlation identifier. All four map a missing reply channel or a lost connection to `RequestError::Transport`, an elapsed timeout to `RequestError::Timeout`, and a remote failure to `RequestError::Remote`. A delivery that violates the wire protocol is refused by `RequestRegistry` before it ever reaches the caller, leaving the slot open for the real reply; such a call observes `RequestError::Timeout`, not `RequestError::Protocol`. `RequestError::Protocol` remains reachable as defense in depth, in the unlikely case a violating delivery is still routed to a live slot. (#399, #439, #445)

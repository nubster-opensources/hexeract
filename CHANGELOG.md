...
- `hexeract-bus`: envelopes can now carry an end-to-end Ed25519 signature over a deterministic canonical representation covering every field, the announced destination and the intended audience. A consumer establishes the publisher identity and rejects a forged, altered or rerouted envelope before any typed decoding. Verification is required by default; an application that deliberately runs without it opts out with `VerificationPolicy::AllowInsecureUnauthenticatedEnvelopes`. Key material comes from a `VerificationKeySource` the application wires, so rotation with an overlap window and explicit revocation are properties of the chosen source. TLS still protects one connection hop only: this protects the message itself. (#444)

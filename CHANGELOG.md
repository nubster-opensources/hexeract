...
- `hexeract-bus`: `RequestClient::close`, an explicit shutdown that rejects new calls, fails every pending call with the new `RequestError::Closed`, and then waits for the reply consumer task to actually stop. That guarantee holds for every clone, including one calling `close` concurrently with another: a caller that finds the supervisor handle already taken waits on the termination signal the consumer raises just before it exits, instead of returning early. Dropping the last handle is the weak half of the same contract: it closes the registry and cancels the token but never awaits, a `drop` that blocks a tokio worker being a deadlock under load. (#399, #401)

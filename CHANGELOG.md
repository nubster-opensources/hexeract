...
- `hexeract-bus-rabbitmq`: two consequences of enabling envelope security, both observable and neither active without it. The six signature headers travel on the same AMQP field table as every other header and count against the same bounds, so an application's own header budget drops from 64 to 58 while a signature is attached. And a signed delivery must carry `message_id`, `correlation_id` and `timestamp` on the wire: these fields are covered by the signature, and the worker previously minted a fresh value for each when it was absent, which no signature scheme survives. They are now a hard rejection under a verifying worker, and a present-but-unparsable value is reported distinctly from an absent one, since the first points at corruption or tampering and the second at an uninstrumented producer. `OutboundEnvelopeSecurity::bind_issuer_to_broker_user` additionally sets the AMQP `user-id` property, which the broker enforces: the broker account must be named exactly as the signed issuer, or every publish is refused by the broker rather than failing verification downstream. (#444)
...
- `hexeract-bus-rabbitmq`: envelope authenticity on the publish and consume paths. `RabbitMqTransport::with_outbound_envelope_security` signs every publish, binding the signature to the routing key it is actually publishing to; `RabbitMqWorkerBuilder::envelope_security` verifies every delivery and establishes the publisher identity before anything settles that delivery, since an acknowledgement is final at the broker and cannot be taken back once a forged message has been accepted.

  **What this makes true, once security is configured**: an envelope that was forged, altered in transit, signed for another audience, or published on a routing key other than the one it was signed for is rejected before any handler runs. The signature binds to the routing key the delivery actually arrived on, never to the destination the envelope announces, which is what lets rerouting be seen at all.

  **What this does not make true**: the payload is not encrypted, so a hostile broker still reads every message; replaying an authentic, unmodified message is not prevented (#447); a handler cannot yet learn who signed what it received, because the verified principal is not exposed to `HandlerContext`; and the request-reply path is **not** covered by this entry, so a reply still travels unsigned and unverified. Signing a reply and verifying it before it resolves a correlation slot come next.

  Nothing changes for an application that configures no envelope security. (#444)

- `hexeract-bus`: `envelope_security`, the primitives for end-to-end envelope authenticity. `EnvelopeSigner` produces an Ed25519 signature over a deterministic canonical representation covering every field of an envelope, the destination it is published to and the audience it is meant for; `EnvelopeVerifier` establishes the publisher identity behind a `VerifiedPrincipal` and rejects a forged, altered or rerouted envelope, before any typed decoding. Verification is required by default; an application that deliberately runs without it opts out with `VerificationPolicy::AllowInsecureUnauthenticatedEnvelopes`, which permits an envelope carrying *no* signature and never ignores one that is present. Key material comes from a `VerificationKeySource` the application wires, so rotation with an overlap window and explicit revocation are properties of the chosen source. TLS still protects one connection hop only: this protects the message itself. The RabbitMQ publish and consume paths call these primitives; see the entry below for what that guarantees and what it does not. (#444)

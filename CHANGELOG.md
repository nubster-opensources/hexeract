...
- `hexeract-bus`: `RequestRegistry` and `PendingReply`, the rendezvous point between a waiting caller and its reply delivery. Registration mints a fresh request id and a drop-safe `PendingReply`, dropping it without waiting always releases its slot, so a cancelled or timed-out request never leaks. (#399)
- `hexeract-bus`: `RequestClient`, a synchronous-over-async RPC client over `Transport`. `request` and `request_with_timeout` publish a `Request` on a fresh causal chain; `request_in` and `request_in_with_timeout` publish it on the causal chain of a caller-supplied `HandlerContext` instead, so a downstream call inherits its correlation identifier. All four map a missing reply channel or a lost connection to `RequestError::Transport`, an elapsed timeout to `RequestError::Timeout`, a wire protocol violation to `RequestError::Protocol`, and a remote failure to `RequestError::Remote`. (#399, #439)

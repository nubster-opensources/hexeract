...
- `hexeract-bus`: `RequestClientSupervisor`, the opaque lifecycle handle for a reply-inbox task. `RequestClientSupervisor::spawn` binds the Tokio task to its completion signal, so a client cannot be built from a supervisor and an unrelated token that would leave concurrent `RequestClient::close` calls waiting forever. A close initiated by the supervisor itself signals shutdown without awaiting its own task; external closers still await actual termination. (#500)

...
| `x-hexeract-deadline` | Implemented | Optional. The request's absolute deadline, as decimal Unix milliseconds in UTC. A responder refuses a request whose deadline lies more than one hour ahead, and still honors one up to one second past, absorbing ordinary clock skew between caller and responder. |
...
| `x-hexeract-request-id` | Which call is this a reply to? | One per request-reply call, never shared even by two concurrent calls on the same chain | The reserved header, minted by `RequestClient` on every call. The responder parses it as a `Uuid` and re-stamps that parsed value on the reply, so a non-canonical form is normalized, not echoed verbatim; a request whose header is absent or fails to parse carries no readable identity and is dropped before the handler runs, with no reply published, so this header is only ever stamped on a reply to a request that carried a readable one (see [Version rules and coexistence](#version-rules-and-coexistence)). |
...
| `Malformed` | The request could not be decoded, or its payload was rejected. Also returned when `x-hexeract-deadline` is present but unreadable or beyond the accepted horizon: an invalid deadline is caller-fixable, correct a clock, shorten a timeout, unlike an unsupported protocol version, which needs a peer upgrade, and the payload carries no free-form text to tell the two apart. |
...
| `Expired` | Reserved: never emitted by this version. A request whose deadline has already passed is dropped silently instead of answered, since its caller has already failed locally; this category exists so a later version is not forced to add one. |

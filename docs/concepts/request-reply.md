...
A deadline is **not** remote cancellation. A handler already running when its deadline passes is never interrupted from the outside. The guard acts at two points instead: it stops a request from being dispatched in the first place when its deadline has already passed on arrival, and, separately, it suppresses the reply of a request whose deadline passed while the handler was still running rather than publishing it. In that second case the handler has already done its work and the request is acked as usual; only the reply, which nothing is left waiting for, is dropped. The counter `reply_dropped_after_deadline` (see [Counters Hexeract exposes directly](../operations/observability.md#counters-hexeract-exposes-directly)) is the only trace of that case anywhere in the system. A handler doing long or segmented work is expected to consult its own remaining time, through `RequestContext::remaining`, and stop early when it chooses to.
...
`RequestError` has eight variants, all reachable from `RequestClient::request` and `RequestClient::request_with`. It is `#[non_exhaustive]`, so a `match` written outside this crate needs a catch-all arm and the compiler will not point out a variant added later: the first three below are local to the caller's own client and are the ones most easily mistaken for one another.

- **`Timeout(Duration)`**: publication and reply waiting did not both complete within one end-to-end local deadline. Time spent acquiring a transport resource, publishing and awaiting broker confirmation reduces the budget left for the reply; publication never receives a separate unbounded phase. The `PendingReply` is dropped with the timed-out request future, so the slot is cleaned up immediately rather than lingering until a reply eventually shows up. An expiry racing broker acceptance is necessarily ambiguous: cancelling the transport future does not prove the broker rejected the request, so the responder may still act and an automatic retry can duplicate execution.
...
- **`Protocol(ProtocolViolation)`**: the call cannot be carried out within the protocol's rules. Three of the four causes concern the reply: a required header is missing or unparsable (`MissingHeader`), the announced protocol version is not implemented by this crate (`UnsupportedVersion`), or the reply's `message_type` does not match what was expected for this call (`UnexpectedReplyType`). The fourth concerns the request instead: `IdentityCollision` means the identity minted for this call was already registered, so registration failed and nothing was published at all. It is the one variant of this error that is safe to retry, and the only one that says the responder never saw the call.
- **`Encode(BusError)`**: the outbound request could not be serialized. This is guaranteed to happen before `Transport::publish_envelope` is called: nothing was published and the responder cannot have acted on the request. The source chain preserves the serialization error, while the top-level message does not expose its details.
...
- **`Decode(BusError)`**: a received reply's payload could not be decoded into `R::Reply` (or `RemoteErrorPayload` for an error reply). The request was necessarily published before that reply could exist, so the responder may already have acted; an automatic retry must account for duplicate execution.

...
- [ ] **Plaintext restricted to local development.** `amqp://` is accepted by default only for `localhost`, `127.0.0.0/8`, and `::1`; remote plaintext is rejected before connecting. Use `amqps://` in production, never `allow_insecure_plaintext_transport`.

...
- [ ] **`outbox apply`/`outbox check` and every `scheduler` admin command use TLS by default.** `outbox apply`/`outbox check` upgrade any `sslmode` other than an explicit `disable` to `require`; `scheduler list`/`inspect`/`dead-letter list`/`dead-letter replay` upgrade an unspecified, `prefer` or `require` mode to `verify-full` on PostgreSQL and `verify-identity` on MySQL (an explicit `verify-ca` is kept as configured). Both families connect against the operating-system trust store, so the same certificate authorities apply everywhere. Only an explicit `disable` (`sslmode=disable`, `ssl-mode=disabled`) opts into plaintext, and a warning naming the redacted target is logged when it does.
